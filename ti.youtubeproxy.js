...
var _getPlaylistsByChannelId = function(channelid) {
    ...
    function handleError(e) { defer.reject(e); }
	(new HTTPRequest({
            url: "https://www.googleapis.com/youtube/v3/playlists?maxResults=50&part=snippet%2CcontentDetails&channelId="+channelid+"&key=" + GOOGLEAPIKEY",
            method: "GET",
...
var _getVideosByplaylistId = function(playlistid) {
    ...
    function handleError(e) { defer.reject(e); }
	(new HTTPRequest({
            url: 'https://www.googleapis.com/youtube/v3/playlistItems?&maxResults=50&part=snippet%2CcontentDetails%2Cstatus&playlistId=' + playlistid + '&key=' + GOOGLEAPIKEY",
            method: "GET",
            success: handleResponse,
            error: handleError,
    })).send();
...
var getUrlById = function(id) {
	var defer = Promise.defer();
	VideoProvider.getVideoUrl('youtube', id).then(
        function onSuccess(url) { defer.resolve(url); }, 
